...
public class LayeredPainter implements PaintListener {
	...
	public static final Integer ZERO_ROOM = new Integer(5);
	public static final Integer ROOM_INTERIOR = new Integer(6);
	public static final Integer SYSTEM_ICON = new Integer(7);
	public static final Integer DOOR = new Integer(8);
	public static final Integer GIB = new Integer(9);
	public static final Integer OVERLAY = new Integer(10);
	public static final Integer ANCHOR = new Integer(11);
	public static final Integer SELECTION = new Integer(12);
	private static Integer[] layerIds = { SHIELD, MOUNT, HULL, GRID, ROOM, ZERO_ROOM, ROOM_INTERIOR, SYSTEM_ICON, DOOR, GIB, OVERLAY, ANCHOR, SELECTION };
	...
	public void remove(PaintBox box) {
		for (Integer layerId : layerIds) {
			if (layerMap.get(layerId).remove(box))
				break;
		}
	}
	
	public void removeAll(PaintBox box) {
		for (Integer layerId : layerIds)
			layerMap.get(layerId).remove(box);
	}

	public boolean contains(PaintBox box) {
		for (Integer layerId : layerIds) {
			if (layerMap.get(layerId).contains(box))
				return true;
		}
		return false;
	}
	
	public boolean contains(PaintBox box, Integer layer) {
		 return layerMap.get(layer).contains(box);
	}
