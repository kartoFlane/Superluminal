package com.kartoflane.superluminal.undo;

import javax.swing.undo.AbstractUndoableEdit;
import javax.swing.undo.CannotRedoException;
import javax.swing.undo.CannotUndoException;

import com.kartoflane.superluminal.elements.FTLMount;
import com.kartoflane.superluminal.painter.PaintBox;

public class UndoableMirrorEdit extends AbstractUndoableEdit {
	private static final long serialVersionUID = -3165329298301115013L;

	private PaintBox box;
	private boolean old;
	
	public UndoableMirrorEdit(PaintBox box) {
		this.box = box;
		old = ((FTLMount) box).isMirrored();
	}
	
	public boolean getOldValue() {
		return old;
	}
	
	public String getPresentationName() {
		return String.format("mirror %s", box.getClass().getSimpleName());
	}
	
	public void undo() throws CannotUndoException {
		super.undo();
		if (box instanceof FTLMount) {
			((FTLMount) box).setMirrored(old);
		}
	}
	
	public void redo() throws CannotRedoException {
		super.redo();
		if (box instanceof FTLMount) {
			((FTLMount) box).setMirrored(!old);
		}
	}
}

