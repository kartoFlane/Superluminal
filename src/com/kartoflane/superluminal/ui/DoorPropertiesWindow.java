...
public class DoorPropertiesWindow {
	...
	public FTLRoom fid = null;
	public FTLRoom sid = null;
	...
	public void setId(FTLRoom room) {
		if (selectingFirst) {
			fid = room;
			lblLeft.setText("" + (fid == null ? "-2" : fid.id));
		} else if (selectingSecond) {
			sid = room;
			lblRight.setText("" + (sid == null ? "-2" : sid.id));
		}
	...
	public void open() {
		...
		if (currentDoor != null) {
			fid = currentDoor.leftRoom;
			sid = currentDoor.rightRoom;
			lblLeft.setText("" + (fid == null ? "-2" : fid.id));
			lblRight.setText("" + (sid == null ? "-2" : sid.id));
		} else {
			lblLeft.setText("");
	...
	private void registerUp(int undoable) {
		if (ume != null) {
			if (undoable == Undoable.DOOR_PROP) {
				((UndoableDoorPropertiesEdit) ume).setCurrentValue(true, ((FTLDoor) currentDoor).leftRoom);
				((UndoableDoorPropertiesEdit) ume).setCurrentValue(false, ((FTLDoor) currentDoor).rightRoom);
				Main.addEdit(ume);
			}
		}
		ume = null;
