...
public class FTLGib extends ImageBox implements Serializable, DraggableBox {
	...
	private String getFreeLetter() {
		...
		for (char ch : alphabet) {
			if (!Main.gibDialog.letters.contains(String.valueOf(ch)))
	...
	public void mouseMove(MouseEvent e) {
		if (move && selected && !Main.animateGibs) {
			if (Main.modShift) { // dragging in one direction, decide direction
				if (Math.pow((bounds.x + offset.x - e.x), 2) + Math.pow((bounds.y + offset.y - e.y), 2) >= 3 && (Main.dragDir == null || Main.dragDir == AxisFlag.BOTH)) { // to prevent picking wrong
																																											// direction due to
